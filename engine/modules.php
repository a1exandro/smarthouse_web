<?php
	class modules
	{
		...
		{
			$board_id = $_SESSION['board_id'];
        	$q = mysql_query("SELECT status FROM boards WHERE id = $board_id");
		...
		public static function updateStat($stat,$m_name)
		{
			...
			$q = mysql_query("UPDATE boards SET status = '$j_stat' WHERE id = $board_id");
		}
		...
		public static function onMsg($msg)
		{
			...
			$data = json_decode(stripslashes($matches[2]));
			
			if ((!$data) || (!$statSender)) return;
